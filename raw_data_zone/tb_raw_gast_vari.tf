resource "aws_glue_catalog_table" "tb_raw_gast_vari" {
  name          = "tb_raw_gast_vari"
  database_name = "db_raw_financas"

  table_type = "EXTERNAL_TABLE"

  storage_descriptor {
    location      = "s3://gusribm-526554803206-analytics-datalake-raw/db_raw_financas/tb_raw_gast_vari/"
    input_format  = "org.apache.hadoop.mapred.TextInputFormat"
    output_format = "org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat"

    ser_de_info {
      name                  = "tb_raw_gast_vari_properties"
    ...
    columns {
      name    = "cod_form_pgto"
      type    = "string"
      comment = "Código da forma de pagamento usada. Pix, Boleto ou 4 dígitos do cartão"
    }

    columns {
      name    = "ano_mes_ref"
      type    = "string"
      comment = "Ano e mÊs de referência da conta no formato YYYY-MM"
    }
  }
}